...
entry_length<-nchar(df$V1[1])
day_latest<-substr(df$V1[1],start=entry_length-12,stop=entry_length-2) 
df$V5[1]<-day_latest
if (nchar(day_latest)==10){
  df$V5[1]<-paste("0",day_latest,sep="")
}
...
mf<-mf[-c(1),] #now remove that column for mf
#change duration to #seconds
y<-seq(1,nrow(mf))
for (val in y){
  hour = format(as.POSIXct(mf$duration[val],format="%H:%M:%S"),"%H")
  hour<-as.numeric(hour) 
  minute = format(as.POSIXct(mf$duration[val],format="%H:%M:%S"),"%M")
  minute<-as.numeric(minute)
  sec = format(as.POSIXct(mf$duration[val],format="%H:%M:%S"),"%S")
  sec<-as.numeric(sec)
